...
# Print configuration
echo "Starting simplified VRF server with WebSocket logsSubscribe..."
...
npm install ws tweetnacl commander
...
# Run with output to temp.log instead of /dev/null
node vrf-server-simple-ws.js \
  ...
  --rpc-url "$RPC_URL" \
  $WS_OPTIONS \
  --scan-interval "$SCAN_INTERVAL" > temp.log 2>&1 &

# Save the PID
echo $! > vrf-server-simple-ws.pid
echo "VRF WebSocket server started with PID $!"
echo "Check temp.log for output" 