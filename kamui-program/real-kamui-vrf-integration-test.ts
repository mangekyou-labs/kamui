...
class RealVRFServer {
    ...
    constructor() {
        ...
        this.cliPath = "./target/debug/ecvrf-cli";
...
describe("Kamui VRF Integration Test - Event Handling & Mangekyou CLI", () => {
    ...
    // Generate a unique seed for this test run
    const uniqueSeedString = `kamui-vrf-integration-${owner.publicKey.toString()}-${Date.now()}`;
        ...
        try {
            ...
            if (subscriptionAccount) {
                ...
                try {
                    ...
                    console.log("❌ Existing subscription corrupted, will recreate");
                }
            }
            ...
            const createSubscriptionIx = new TransactionInstruction({
                keys: [
                    ...
                    { pubkey: cleanSeed.publicKey, isSigner: false, isWritable: false },
