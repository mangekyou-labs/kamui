...
import { KamuiVrf, IDL as VrfIDL } from '../target/types/kamui_vrf';
import { KamuiVrfConsumer, IDL as ConsumerIDL } from '../target/types/kamui_vrf_consumer';
...
describe('Kamui VRF Integration Tests', () => {
    ...
    // Define program IDs
    const vrfProgramId = new PublicKey("DfAMyTXQ6wSJRk5hLW6dBSkBWGGYr6BXzJT9Wx8v8XZ");
    const consumerProgramId = new PublicKey("GwLtp4BFQfzHSAF2oZ1PQSgFTV21yoKLDJZGifKwRR7o");

    // Load programs using IDL directly instead of workspace
    const vrfProgram = new anchor.Program<KamuiVrf>(
        VrfIDL,
        provider,
        { programId: vrfProgramId }
    );

    const consumerProgram = new anchor.Program<KamuiVrfConsumer>(
        ConsumerIDL,
        provider,
    ...
    it('Fulfills randomness with direct proof generation', async () => {
        ...
        seed.copy(mockProof, 0, 0, 32);     // gamma = seed
        seed.copy(mockProof, 32, 0, 16);    // c = first 16 bytes of seed
        nacl.randomBytes(32).copy(mockProof, 48); // s = random
