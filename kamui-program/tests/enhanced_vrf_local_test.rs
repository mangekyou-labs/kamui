...
use kamui_program::{
    ...
    processor::Processor,
    state::{EnhancedOracle, EnhancedSubscription, OracleRegistry, RequestPool, RequestStatus},
};
...
async fn test_enhanced_vrf_direct_proof() {
    ...
    let mut program_test =
        ProgramTest::new("kamui_program", program_id, processor!(Processor::process));
    ...
    let (oracle_config_pda, _) =
        find_program_address(&[b"oracle_config", oracle.pubkey().as_ref()], &program_id);
    ...
    let (subscription_pda, _) =
        find_program_address(&[b"subscription", &subscription_seed], &program_id);
    ...
    let ix = Instruction {
        ...
        data: VrfCoordinatorInstruction::FundSubscription { amount: 50_000_000 }
            .try_to_vec()
            .unwrap(),
    };
    ...
    let (pool_pda, _) = find_program_address(
        &[b"request_pool", subscription_pda.as_ref(), &[pool_id]],
        &program_id,
    );
    ...
    let (request_pda, _) = find_program_address(&[b"vrf_request", &request_id], &program_id);
    ...
    let (result_pda, _) = find_program_address(&[b"vrf_result", request_pda.as_ref()], &program_id);
    ...
    let ix = Instruction {
        ...
        data: VrfCoordinatorInstruction::CleanExpiredRequests { pool_id }
            .try_to_vec()
            .unwrap(),
    };
