use {
    ...
    hex,
    kamui_program::{
        instruction::{VerifyVrfInput, VrfCoordinatorInstruction},
        state::Subscription,
    },
    mangekyou::kamui_vrf::{
...
async fn start_vrf_transaction_monitor(program_id: Pubkey) -> Result<(), anyhow::Error> {
    ...
    let pubsub_client = PubsubClient::new(&ws_url)
        .await
        .map_err(|e| anyhow::anyhow!("PubSub client error: {:?}", e))?;

    println!(
        "WebSocket connected. Subscribing to logs for program: {}",
        program_id
    );
...
async fn send_test_transaction(
    ...
    match result {
        Ok(sig) => {
            println!(
                "Test transaction unexpectedly succeeded! Signature: {}",
                sig
            );
            Ok(sig.to_string())
        }
...
async fn test_vrf_flow_devnet() -> Result<()> {
    ...
    start_vrf_transaction_monitor(vrf_program_id)
        .await
        .expect("Failed to start VRF transaction monitor");
    ...
    let signature = send_transaction_with_retries(
        &rpc_client,
        &transaction,
        &[&payer, &subscription_owner, &subscription_account],
        5,
    ...
    let transfer_ix =
        system_instruction::transfer(&subscription_owner.pubkey(), &funder_token, wrap_amount);
    let sync_native_ix = spl_token::instruction::sync_native(&spl_token::id(), &funder_token)?;

    let recent_blockhash = rpc_client
        .get_latest_blockhash()
        .expect("Failed to get recent blockhash");
    let mut transaction =
        Transaction::new_with_payer(&[transfer_ix, sync_native_ix], Some(&payer.pubkey()));
    transaction.sign(&[&payer, &subscription_owner], recent_blockhash);

    println!("Wrapping SOL...");
    let signature =
        send_transaction_with_retries(&rpc_client, &transaction, &[&payer, &subscription_owner], 5)
    ...
    let recent_blockhash = rpc_client
        .get_latest_blockhash()
        .expect("Failed to get recent blockhash");
    let mut transaction = Transaction::new_with_payer(&[init_game_ix], Some(&payer.pubkey()));
    transaction.sign(&[&payer, &game_owner], recent_blockhash);

    println!("Initializing game state...");
    let signature =
        send_transaction_with_retries(&rpc_client, &transaction, &[&payer, &game_owner], 5)
    ...
    println!(
        "Initial payer balance: {} SOL",
        initial_payer_balance as f64 / 1_000_000_000.0
    );
    println!(
        "Initial game owner balance: {} SOL",
        initial_game_owner_balance as f64 / 1_000_000_000.0
    );
    ...
    let mut transaction = Transaction::new_with_payer(&[request_vrf_ix], Some(&payer.pubkey()));
    let recent_blockhash = rpc_client
        .get_latest_blockhash()
        .expect("Failed to get recent blockhash");
    transaction.sign(
    ...
    println!(
        "Request cost: {} SOL",
        request_cost as f64 / 1_000_000_000.0
    );
    ...
    while !result_account_exists && attempts < max_attempts {
        match rpc_client.get_account(&vrf_result) {
            Ok(account) => {
                ...
                println!(
                    "VRF result account created! Data length: {}",
                    account.data.len()
                );
            ...
            Err(err) => {
                if attempts % 5 == 0 {
                    ...
                    match rpc_client.get_account(&request_account) {
                        Ok(req_account) => {
                            ...
                            if req_account.data.len() > 136 {
                                ...
                                println!(
                                    "Request account exists, approximate status byte: {}",
                                    status_approx
                                );
                                ...
                                if req_account.data.len() >= 150 {
                                    println!(
                                        "Request data bytes 130-150: {:?}",
                                        &req_account.data[130..150]
                                    );
                                }
                            }
                        }
                    ...
                    println!(
                        "Waited for {} seconds, {} seconds remaining...",
                        attempts,
    ...
    if !result_account_exists {
        ...
        match rpc_client.get_account(&subscription_account.pubkey()) {
            Ok(sub_account) => {
                println!(
                    "Subscription account exists, data length: {}",
                    sub_account.data.len()
                );
        ...
        match rpc_client.get_account(&request_account) {
            Ok(req_account) => {
                println!(
                    "Request account exists, data length: {}",
                    req_account.data.len()
                );
                if req_account.data.len() > 136 {
    ...
    println!(
        "Fulfill cost: {} SOL",
        fulfill_cost as f64 / 1_000_000_000.0
    );
    ...
    println!(
        "Consume cost: {} SOL",
        consume_cost as f64 / 1_000_000_000.0
    );
    ...
    println!(
        "Total VRF cost: {} SOL",
        total_vrf_cost as f64 / 1_000_000_000.0
    );
    println!("==================\n");
        ...
        match rpc_client.get_account(&vrf_result) {
            Ok(account) => {
                ...
                if account.data.len() > 0 {
                    println!(
                        "VRF result account exists with data length: {}",
                        account.data.len()
                    );
                    if attempts % 5 == 0 {
    ...
    println!("\n=== Cost Comparison ===");
    println!(
        "First request total cost:  {} SOL",
        total_vrf_cost as f64 / 1_000_000_000.0
    );
    println!(
        "Second request cost:       {} SOL",
        second_request_cost as f64 / 1_000_000_000.0
    );
    println!(
        "Cost reduction:            {:.2}%",
